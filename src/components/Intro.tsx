  ...
  return (
    <div className="relative z-10 flex min-h-[calc(100vh-72px)] flex-col items-center justify-center px-6 py-12 pb-20 pt-12">
      <div className="relative z-10 w-full max-w-[740px] animate-[fadeUp_0.6s_ease_both] text-center">
        <div className="mb-8 inline-flex items-center gap-2.5 rounded border border-amber-500/25 bg-amber-dim px-4 py-1.5 font-head text-[13px] font-semibold uppercase tracking-[0.08em] text-amber-light">
          <span className="h-1.5 w-1.5 shrink-0 animate-pulse rounded-full bg-amber" />
          Free 3-Minute Assessment
        </div>
        <h1 className="mb-7 font-head text-[clamp(48px,7vw,82px)] font-black leading-[0.96] tracking-[-0.01em] uppercase">
          How Dependent
          <br />
        ...
        <div className="mx-auto mb-9 flex max-w-[560px] border border-divider">
          {[
            ...
            >
              <span className="block font-head text-4xl font-black leading-none text-amber">{num}</span>
        ...
        <button
          onClick={onStart}
          className="inline-block border-2 border-amber bg-amber px-9 py-4 font-head text-[17px] font-extrabold uppercase tracking-[0.06em] text-ink transition-all hover:-translate-y-0.5 hover:bg-transparent hover:text-amber"
        >
          Find Out Now →
        </button>
